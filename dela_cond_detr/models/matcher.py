...
from dela_cond_detr.util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def get_cost(self, outputs, tgt_ids, tgt_bbox):
        ...
        alpha = 0.25
        gamma = 2.0
        neg_cost_class = (1 - alpha) * (out_prob ** gamma) * (-(1 - out_prob + 1e-8).log())  # [bs * num_query, num_cls]
        pos_cost_class = alpha * ((1 - out_prob) ** gamma) * (-(out_prob + 1e-8).log())  # [bs * num_query, num_cls]
        cost_class = pos_cost_class[:, tgt_ids] - neg_cost_class[:, tgt_ids]  # [bs * num_query, batch_inst]

        # Compute the L1 cost between boxes
        cost_bbox = torch.cdist(out_bbox, tgt_bbox, p=1)  # [bs * num_query, batch_inst]

        # Compute the giou cost betwen boxes
        cost_giou = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))

        # Final cost matrix
        # additional computations are made to avoid for loop and enhance parallel
        C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou
        C = C.view(bs, num_queries, -1).cpu()  # [bs, num_query, batch_inst]
    ...
    def forward_single_stage(self, outputs, targets):
        ...
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]  # c[i]: [nq, num_inst[i]]
