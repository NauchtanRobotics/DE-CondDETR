...
def get_args_parser():
    ...
    parser.add_argument('--pre_nms', type=int, default=2000, help="number of top score pred before nms")

    # customized model
    parser.add_argument('--pool_res', type=int, default=4, help="roi size (pooler_resolution)")
...
def main(args):
    ...
    if utils.get_rank() == 0:
        if args.wandb:
            wandb.init(config=args, project="DE-CondDETR")
    ...
    if args.eval:
        ...
        print('{}, {}, {}, {}, {}, {}'.format(results[0], results[1], results[2], results[3], results[4], results[5]))
        if args.output_dir:
