...
class ConditionalDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False,
                 box_refine=False, num_feature_levels=3):
        ...
        if num_feature_levels > 1:
            ...
            self.input_proj = nn.ModuleList([
                nn.Sequential(
                    ...
                    # nn.GroupNorm(32, hidden_dim),  # for fair comparison
                )])
    ...
    def forward(self, samples: NestedTensor, meta_info):
        ...
        hs, reference, outputs_coord = self.transformer(
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # since the bbox are normalized, the L1 loss values in [0, 1]
        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction='none')

        losses = {}
        losses['loss_bbox'] = loss_bbox.sum() / num_boxes

        # giou loss values in [-1, 1]
...
class PostProcess(nn.Module):
    ...
    def __init__(self, nms=False, nms_thresh=0.7, pre_nms=2000):
        super().__init__()
        self.nms = nms
    ...
    def forward(self, outputs, target_sizes):
        if self.nms:
            return self.forward_nms(outputs, target_sizes)
        else:
            return self.forward_base(outputs, target_sizes)

    @torch.no_grad()
    def forward_base(self, outputs, target_sizes):
    ...
    @torch.no_grad()
    def forward_nms(self, outputs, target_sizes):
        """ Perform the computation with nms
        TODO: no need to create two forward function, merge with forward_base
        Parameters:
            outputs: raw outputs of the model
            target_sizes: tensor of dimension [batch_size x 2] containing the size of each images of the batch
                          For evaluation, this must be the original image size (before any data augmentation)
                          For visualization, this should be the image size after data augment, but before padding
        """
        out_logits, out_bbox = outputs['pred_logits'], outputs['pred_boxes']  # (bs, 300, #cls), (bs, 300, 4)

        assert len(out_logits) == len(target_sizes)
        assert target_sizes.shape[1] == 2

        prob = out_logits.sigmoid()
        topk_values, topk_indexes = torch.topk(prob.view(out_logits.shape[0], -1), self.pre_nms, dim=1)
        ...
        for (bbox_, score_, bbox_idx) in zip(out_bbox, prob_cls, labels):
            keep_ = batched_nms(bbox_, score_, bbox_idx, iou_threshold=self.nms_thresh)
            assert len(keep_) >= 100, "only {} left after nms with iou thresh={}".format(len(keep_), self.nms_thresh)
...
def build(args):
    ...
    postprocessors = {'bbox': PostProcess(
        nms=args.nms, nms_thresh=args.nms_thresh, pre_nms=args.pre_nms,
    )}
    if args.masks:
